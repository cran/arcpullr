...
the respective `get_layer_by_*` function.
...
For multiple WHERE clauses there is a helper function to string them
together properly. This can be called to the where argument above in
place of a text string.
...
# The `get_map_layer()` function

This function retrieves map service layers from an ArcGIS REST services
API and returns them as a `RasterLayer` object (*i.e.* of the R package
[raster: Geographic Data Analysis and Modeling](https://CRAN.R-project.org/package=raster)).

These layers require a bounding box to query the map layer, which is
either taken from the `sf_object` argument or optionally can be passed
via the `bbox` argument.
...
The URL should be a specific raster layer from an ArcGIS REST API, such
as the [Wisconsin Dep't. of Natural Resources Wiscland
Layer](https://dnrmaps.wi.gov/arcgis_image/rest/services/DW_Land_Cover/EN_Land_Cover2_Lev2/MapServer). <br> 

\
\

# The `get_image_layer` function

This function retrieves image service layers from an ArcGIS REST
services API and returns them as a `RasterStack` object.
...
The URL should be a specific image layer from an ArcGIS REST API, such
as the [Wisconsin Dep't. of Natural Resources Leaf Off
Basemap](https://dnrmaps.wi.gov/arcgis_image/rest/services/DW_Image/EN_Image_Basemap_Leaf_Off/ImageServer).
